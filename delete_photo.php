...
<body> 
    <h4>Code xóa toàn bộ ảnh</h4>
    <h5>Không xóa được ảnh bìa, ảnh đại diện, ảnh được tag</h5>
    <form method="post">
...
if(isset($_POST['ok'])){
    ...
    if($option==0){
        $since = $_POST['since'];
        $until = $_POST['until'];
        $link  = "https://graph.facebook.com/me/photos?fields=id&limit=2500&access_token=$token&since=$since&until=$until";
    }
    else{
       $link = "https://graph.facebook.com/me/photos?fields=id&limit=2500&access_token=$token"; 
    }
    while (true) {
       $curl    = curl_init();
